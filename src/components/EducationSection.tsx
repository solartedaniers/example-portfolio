...
export default function EducationSection({ dict }: EducationSectionProps) {
  ...
  return (
    <section className="flex flex-col gap-4 items-center col-span-6 row-span-3 
                        rounded-2xl bg-gray-200 dark:bg-gray-800 p-10">
      {Object.values(edu).map((item, idx) => (
        <article
          key={idx}
          className={`flex items-center justify-between w-full ${
            idx < Object.values(edu).length - 1 ? 'border-b border-gray-400 pb-8' : ''
          }`}
        >
