...
export default function InterestsSection({ dict }: InterestsSectionProps) {
  return (
    <ul className="flex items-center col-span-9 gap-4 p-5 rounded-2xl 
      ...
      {dict.sections.interests.items.map((item) => (
        <li
          key={item}
          className="flex items-center bg-gray-300 dark:bg-gray-900 rounded-lg 
                     text-gray-900 dark:text-white text-sm px-4 py-2"
        >
          {item}
        </li>
      ))}
